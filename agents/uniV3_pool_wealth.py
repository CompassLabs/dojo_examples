...
class UniV3PoolWealthAgent(BaseAgent):
    ...
    def __init__(self, initial_portfolio: dict, name: str = "UniV3PoolWealthAgent"):
    ...
    def reward(self, obs: UniV3Obs) -> float:
        ...
        # print("lp portfolio", lp_portfolio)
        # print("wallet portfolio", wallet_portfolio)
        # total_portfolio = {**lp_portfolio, **wallet_portfolio}

        # wealth expressed as token0 of the pool
        wealth = 0
        # print(total_portfolio.items())
        for token, quantity in wallet_portfolio.items():
            wealth += quantity * obs.price(
                token=token, unit=pool_tokens[0], pool_name=pool
            )
